...
def schluesselDefinieren():
    ...
    N = p*q
    a = (p-1)*(q-1)
    e = simpledialog.askinteger(
        'Encipher Zahl', 'Was ist deine "encipher" Zahl? (e)', initialvalue=27)
    bb = e              # ggT herausfinden
    aa = a
    time = 0
    while not bb == 0 and time < timeTriggerGGT:
...
#        print('rest:',rest)
    while 1 > e or e >= N or aa != 1:                # Zahl 'e' prüfen
        if aa != 1:
            #            msgDlg('')
            e = inputInt(
                'Du darfst keine Zahle, die den ggt von ihr und "p*q" , 1 gibt gebrauchen.\nBitte gib eine neue Zahle für "e" ein.\nWas ist deine "encipher" Zahl? (e)')
            bb = e              # ggT herausfinden
            aa = a
            time = 0
            while not bb == 0 and time < timeTriggerGGT:
...
#                print('rest:',rest)
        if 1 > e:
            #            msgDlg('')
            e = inputInt(
                'Du darfst keine Zahle kleiner als 1 auswählen.\nBitte gib eine neue Zahle für "e" ein.\nWas ist deine "encipher" Zahl? (e)')
        if e >= N:
            #            msgDlg('')
            e = inputInt(
                'Du darfst keine Zahle grösser als "p*q" auswählen.\nBitte gib eine neue Zahle für "e" ein.\nWas ist deine "encipher" Zahl? (e)')
# setColor('WHITE')
    fillRectangle(feldTextVerschluesseln[0], feldTextVerschluesseln[1],
                  feldTextVerschluesseln[2], feldTextVerschluesseln[3])
# setColor('BLACK')
...
schluesselDefinierenKnopf.place(x=window_x_start+window_borderwidth,
                                y=window_height-window_borderwidth -
...
verschluesselnKnopf.place(
    x=window_width/2-feldTextVerschluesseln[3]/2, y=feldTextVerschluesseln[1], width=feldTextVerschluesseln[3], height=feldTextVerschluesseln[2])
...
entschluesselnKnopf.place(
    x=window_width/2-feldTextEntschluesseln[3]/2, y=feldTextEntschluesseln[1], width=feldTextEntschluesseln[3], height=feldTextEntschluesseln[2])
