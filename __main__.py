# import RSAPythonv2 as RSA_old
# import RSA_final as RSA
from variables import root_x_start, root_y_start, root_width, root_height, root_borderwidth
...
# RSA_old.root.mainloop()

def nochNichtProgrammiert():
    messagebox.showerror('Verschlüsselungs editor', 'Ist noch nicht programmiert!')
...
class RSA_main(tk.Frame):
    ...
    # def start(self):
        ...
        schluesselDefinierenKnopf = tk.Button(rsa_window, text=feldSchluesselSelbstDefinieren[4], command=self.schluesselDefinieren,
        ...
        schluesselDefinierenKnopf.place(x = root_x_start+root_borderwidth,
                                        y=root_height-root_borderwidth-feldSchluesselSelbstDefinieren[2],
                                        width=feldSchluesselSelbstDefinieren[3], height=feldSchluesselSelbstDefinieren[2])

        schluesselDefinierenAutomatischKnopf = tk.Button(rsa_window, text=feldSchluesselAutomatischDefinieren[4], command=self.schluesselDefinierenAutomatisch,
                                                    background='#FFFF00', activebackground='#FFF000', cursor='exchange', borderwidth=5)
        schluesselDefinierenAutomatischKnopf.place(x = root_width-root_borderwidth-feldSchluesselAutomatischDefinieren[3],
                                                y=root_height-root_borderwidth-feldSchluesselAutomatischDefinieren[2],
                                                width=feldSchluesselAutomatischDefinieren[3], height=feldSchluesselAutomatischDefinieren[2])

        verschluesselnKnopf = tk.Button(rsa_window, text=feldTextVerschluesseln[4], command=self.verschluesseln,
                                    background='#7CFC00', activebackground='#80FF07',  cursor='exchange', borderwidth=5)
        verschluesselnKnopf.place(x=root_width/2-feldTextVerschluesseln[3]/2, y=feldTextVerschluesseln[1], width=feldTextVerschluesseln[3], height=feldTextVerschluesseln[2])

        entschluesselnKnopf = tk.Button(rsa_window, text=feldTextEntschluesseln[4], command=self.entschluesseln,
                                    background='#40E0D0', activebackground='#48D1CC', cursor='exchange', borderwidth=5)
        entschluesselnKnopf.place(x=root_width/2-feldTextEntschluesseln[3]/2, y=feldTextEntschluesseln[1], width=feldTextEntschluesseln[3], height=feldTextEntschluesseln[2])
        rsa_window.mainloop()
    ...
    def primzahl_pruefen(self, zahl, random_prime, anzahl=1):
        ...
        if random_prime == True:
            ...
            if zahl <= 1:
                message = ["Die von dir eingegebene Zahl ", str(zahl), " ist keine Primzahl.\nBitte eine ander eingeben."]
    ...
    def eingabe(self, random_prime, anzahl=1):
        if random_prime == True:
            ...
            if anzahl == 1:
                n = Dialogs.input_Int(root, "Primzahl eintippen","Was ist deine erste Primzahl? (p)","11")
                n = self.primzahl_pruefen(n, random_prime, anzahl)
            elif anzahl == 2:
                n = Dialogs.input_Int(root, "Primzahl eintippen","Was ist deine zweite Primzahl? (q)","5")
                n = self.primzahl_pruefen(n, random_prime, anzahl)
        return n
    ...
    def schluesselDefinierenAutomatisch(self):
        ...
        global g
        global d
        h = Dialogs.input_Int(root, 'RSA','Was ist die tiefste Zufallszahl?',"10")
        g = Dialogs.input_Int(root, 'RSA','Was ist die höchste Zufallszahl?',"100")
        ...
        message = ["d = ", str(d)]
        message = "".join(message)
        string_d = tk.Label(rsa_window, text=message)
    ...
    def schluesselDefinieren(self):
        ...
        while p == q:
            messagebox.showerror('RSA', 'Du darfst keine gleichen Zahlen auswählen.\nBitte gib neue Zahlen ein.')
            q = self.eingabe(False, 2) # Eingabe von Q
        N = p*q
        a = (p-1)*(q-1)
        e = Dialogs.input_Int(root, 'Encipher Zahl','Was ist deine "encipher" Zahl? (e)', "27")
        bb = e # ggT herausfinden
        aa = a
        time = 0
        while not bb == 0 and time < timeTriggerGGT:
        ...
        while 1 > e or e >= N or aa != 1: # Zahl 'e' prüfen
            if aa != 1:
                message = []
                e = Dialogs.input_Int(root, "Encipher Zahl", 'Du darfst keine Zahle, die den ggt von ihr und "p*q" , 1 gibt gebrauchen.\nBitte gib eine neue Zahle für "e" ein.\nWas ist deine "encipher" Zahl? (e)', "")
                bb = e # ggT herausfinden
                aa = a
                time = 0
                while not bb == 0 and time < timeTriggerGGT:
                    ...
                    bb = rest
            if 1 > e:
                e = Dialogs.input_Int(root, "Encipher Zahl", 'Du darfst keine Zahle kleiner als 1 auswählen.\nBitte gib eine neue Zahle für "e" ein.\nWas ist deine "encipher" Zahl? (e)', "")
            if e >= N:
                e = Dialogs.input_Int(root, "Encipher Zahl", 'Du darfst keine Zahle grösser als "p*q" auswählen.\nBitte gib eine neue Zahle für "e" ein.\nWas ist deine "encipher" Zahl? (e)', "")
        message = ["e = ", str(e), "N = ", str(N)]
        message = "".join(message)
        string_e_N = tk.Label(root, text=message)
        string_e_N.place(x = 250, y = 60, width = 150, height = 80)
    ...
    def verschluesseln(self):
        mString = Dialogs.input_Str(root, "Verschlüsselung", 'Was wollen sie verschlüsseln?', "Hallo")
        m = []
        if letters_sepperately == True:
            ...
            for n in range(len(m)):
                C.append(m[n]**e % N)
            # C = (m**e) % N
        Dialogs.input_Str(root, "Verschlüsselung", "Ihr verschlüsselter Text lautet:", C)
        # messagebox.showinfo('Ihr verschlüsselter Text lautet:\n', C) 
        
    def entschluesseln(self):
        C = Dialogs.input_Str(root, "Verschlüsselung", 'Was wollen sie entschlüsseln?', "")
        C_list = []
        first = 0
        ...
        Dialogs.input_Str(root, "Verschlüsselung", "Ihr entschlüsselter Text lautet:", end_message)
...
if __name__ == "__main__":
    ...
    firstMenu.add_separator()
    firstMenu.add_command(label="RSA", command=RSA_command)
    root.mainloop()
